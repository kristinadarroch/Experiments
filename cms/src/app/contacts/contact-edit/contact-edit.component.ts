...
export class ContactEditComponent implements OnInit {
  ...
  isInvalidContact(newContact: Contact): boolean {
    if (!newContact) { // no contact passed?
      return true;
    }

    if (newContact.id === this.contact.id) { // adding contact to self?
      return true; // adding contact to self?
    }

    for (let currContact of this.groupContacts) {
      if (currContact.id === newContact.id) {
        return true; // contact already in group
      }
    }
  ...
  addToGroup($event: any) {
    ...
    if (this.isInvalidContact(selectedContact)) {
      ...
      return; // do not add to group
    }
    // create a clone of the contact object
